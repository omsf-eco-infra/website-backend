...
@mock_aws
def test_publish_sqs_message_accepts_inline_payload() -> None:
    sqs = boto3.client("sqs", region_name="us-east-1")
    queue_url = sqs.create_queue(
        QueueName="example.fifo",
        Attributes={"FifoQueue": "true", "ContentBasedDeduplication": "false"},
    ...
    assert len(messages) == 1
    assert messages[0]["Body"] == '{"hello": "inline"}'


def test_publish_sqs_message_requires_exactly_one_payload_source(
    tmp_path: Path,
) -> None:
    payload_file = tmp_path / "payload.json"
    payload_file.write_text(json.dumps({"hello": "world"}), encoding="utf-8")

    with pytest.raises(ValueError, match="Exactly one of payload_file or payload"):
        publish_sqs_message.publish_message(
